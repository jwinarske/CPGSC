...
FUNCTION NAME:  s_fgetc
 ...
 PROTOTYPE IN:  BUOS.H
  ...
  DESCRIPTION:  Provides "formatted" character-by-character serial input
                ...
                SIO.
      RETURNS:  int: TIMEOUT if t_outflag is TRUE and no byte
                arrives in t_out Tsecs; IGNORE if an EOL decision
                pending; otherwise the byte received.
...
int s_fgetc(SIO *siop)
{
     ...
     if (ip->rbakflag)                        /* if byte pending              */
          {
          ip->rbakflag = FALSE;
          if (ip->consflag)
               putchar(ip->rbakc);
     ...
     if (ip->asciiflag)                       /* -- RESET BIT 7 -- */
          c = toascii(c);
     if (ip->filterflag)                      /* -- TRANSLATE/REMOVE BYTE -- */
          {
     ...
     if (retbyte != -1)            /* if byte to return is not byte received */
          c = retbyte;
