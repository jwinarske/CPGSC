/*-34
    PROGRAM:  RUN2BMAP
DESCRIPTION:  Converts runlength file to 216-bytes per line bitmap.
...
void main(void)
{
    ...
    int   errcode = 0;
    BYTE *bitbufp;               /* Points to the buffer filled with data */
    short *runbufp;
    short bitbuf_len = 608;
    short run_len = BYTES_PER_LINE;
    if ((bitbufp  = (BYTE*)malloc(WIDTH * bitbuf_len)) == NIL)
