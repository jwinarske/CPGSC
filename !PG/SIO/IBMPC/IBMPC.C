...
FUNCTION NAME:  __s_waitch
 PROTOTYPE IN:  IBMPC.H
        LEVEL:  0
      LIBRARY:  IBMPC.C
  DESCRIPTION:  Waits for a serial byte to arrive for tsecs Tsecs.
      RETURNS:  int, the byte if one arrives, otherwise NOT_REAY at timeout.
...
int __s_waitch(register SIO *siop, unsigned tsecs)
{
     ...
     while (tsecs > 0)
          {
          if ( (*siop->s_rstat)(siop) & siop->rcv_vflowstate) /* check UART for byte ready         */
               return  ((*siop->s_read)(siop));  /* return byte if ready     */
     ...
     return s_inchar(siop);                   /* timed out--no byte received */
}
...
void __sysintron(SIO *siop, RANK portnum)
{
     ...
     __sys_ilist[portnum].oldimr =  imr = _a_inport ((BYTE*)IMR);
     imr = (BYTE) (imr &  __sys_ilist[portnum].imask);
     _a_outport((BYTE*) IMR, imr);                  
...
{
     __sys_disable();
     _a_outport( (BYTE*)IMR, __sys_ilist[portnum].oldimr); /* restore Int.Mask Register */
     __sys_enable();
