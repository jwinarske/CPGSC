...
FUNCTION NAME:  m_cmd_str
 ...
 PROTOTYPE IN:  MODEM.H
  ...
  DESCRIPTION:  Outputs a command, then fetches the modem's response.
                The maximum number of bytes to be fetched is specified in
                numc.
      RETURNS:  short: the number of characters in the modem's response,
                zero if no response.
     COMMENTS:  Any echoed bytes are cleared from the SIO between
...
short m_cmd_str(SIO *siop, char* cmdstr, char* replybuff, USHORT numc)
{
     ...
     s_clrsio(siop, strlen(cmdstr));         /* clear echo           */
