...
FUNCTION NAME:  isr_rcv450
 PROTOTYPE IN:  U16X50.H
        LEVEL:  1
      LIBRARY:  SIO.LIB
  DESCRIPTION:  Byte-read Interrupt subhandler for 16450/8250 (index 2)
                interrupts. Also used for 16550 FIFO timeout 
                interrupt (index 6).
      RETURNS:  void
     COMMENTS:  Reads a byte from the UART's FIFO, places it in the 
...
FUNCTION NAME:  isr_rcv550
 PROTOTYPE IN:  U16X50.H
        LEVEL:  1
      LIBRARY:  SIO.LIB
  DESCRIPTION:  Multiple byte-read interrupt subhandler for 16550 
...
{
     short fifo_bytes = siop->rcv_fifo_trigger;         /* Num bytes in FIFO */
     while (fifo_bytes--)                /* While there's a byte in the FIFO */
       {
       *siop->rcv_headp = s_rcv(siop);                      /* Read receiver */
       if ( ++siop->rcv_headp == siop->rcv_buffend)   /* If at end of buffer */
...
{
   int diff;
   static int bytes_left = FLOW_HEADROOM +1;
...
{
   int diff;
   static int bytes_left = FLOW_HEADROOM;
