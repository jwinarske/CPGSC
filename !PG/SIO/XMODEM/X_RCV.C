...
int x_rcv(SIO *siop)
{
     extern USHORT (*r_errckp)(USHORT*); /* pointer to the error check funct */
     extern struct rcvpacket *rcvbuffp;    /* buffer pointer is global    */
     extern USHORT paksize;          /* number of bytes to xmit or rcv    */
...
/* ----- static locals for speed of access ----- */
     ...
     static USHORT         i, j;
     static USHORT        *intp;     /* for addressing structure as array */
     static unsigned  long paktot;   /* running packet total              */
     static USHORT         numpax;   /* number of packets in buffer       */
     static USHORT         buffpax;  /* number of packets received so far */
     static int            soh;      /* received start of header byte     */
/* -- */
     ...
     int  errcnt;                    /* main exception counter     */
     int  nosohtot;                  /* special counter for soh timeouts  */
     ...
     printf("Buffer = %u packets (%lu bytes).\n", numpax, (long)numpax * DBLKSIZ);
...
/* ------------------------------------------------------------- */
     ...
     csync    = (BYTE) ((r_errckp == x_rcvcrc) ? 'C': NAK); /* match r_errckp   */
               ...
               switch (soh)
                    ...
                    case EOT:                /* no more packets        */
                         ...
                         if (fflush(rfp) == 0)
                              ...
                              printf(" of %s:  %lu bytes (%lu blocks).\
