...
/* ----- static locals for speed of access ----- */
     ...
     static BYTE     *bytep;    /* for addressing structure as array */
     static USHORT ack, i;
     static USHORT numpax;      /* buffer size in packets            */
     static USHORT paxmade;     /* number of packets received so far */
     static USHORT (*s_errckp)();       /* pointer to checkval funct */
...
/* -- */
     ...
     printf("Buffer size = %u packets (%lu bytes).\n", numpax, (long)numpax * CRCPAKSIZ );
...
/* --------------------------------------*/
     ...
     while (errcode == CONTINUE)       /* BEGIN MAIN TRANSMIT LOOP     */
          ...
          for (i = 0, bytep = (BYTE *)sndbuffp; i < paksize; i++)
               {
               s_putc(siop, *bytep++);           /* SEND IT !          */
               }
#endif
          s_write(siop, (BYTE *) sndbuffp, paksize);
