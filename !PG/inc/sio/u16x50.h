...
void (*s_revector(SIO *, uint16_t, void (*)(SIO *) )) (SIO*);
...
enum vir_row {
    	VBAUDLO = 0,           /* lo uint8_t of baud rate register       */
        VBAUDHI,               /* hi uint8_t of baud rate register       */
...
#define  RCV_MASK       0x01     /* 00000001 received uint8_t ready      */
#define  XMIT_MASK      0x20     /* 00000010 transmitter buffer clear */
#define  TX_EMPTY_MASK  0x40     /* 00000100 shift register clear     */

#define s_txempty(x) (*(x)->readuint8_t)((x)->uart_base+(x)->status_off)&(x)->tx_emptymask
