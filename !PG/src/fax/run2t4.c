...
int main(void)
{
     ...
     uint16_t rl_read;                     /* Run lengths read from disk   */
     uint16_t t4_written;                  /* Number of T4 codes in written*/
     uint16_t read_total, write_total;     /* Statistics                   */
...
/* Read run lengths, encode T.4, write T.4 codes */
     ...
     printf("\n%d Runlength uint8_ts read\n", read_total * 2);
     printf("%d T4 uint8_ts written\n", write_total * 2);
