...
#pragma check_stack-                 /* turn off stack checking */
       ...
       #ifdef COM1SIO
#include "\!pg\sio\ibmpc\COM1SIO.C"
       #endif

       #ifdef COM2SIO
#include "\!pg\sio\ibmpc\COM2SIO.C"
       #endif

       #ifdef COM3SIO
#include "\!pg\sio\ibmpc\COM3SIO.C"
       #endif


       #ifdef COM4SIO
#include "\!pg\sio\ibmpc\COM4SIO.C"
       #endif
...
FUNCTION NAME:  __s_waitch
 PROTOTYPE IN:  IBMPC.H
        LEVEL:  0
      LIBRARY:  IBMPC.C
  DESCRIPTION:  Waits for a serial uint8 to arrive for tsecs Tsecs.
      RETURNS:  int, the uint8 if one arrives, otherwise NOT_REAY at timeout.
...
int __s_waitch(register SIO *siop, unsigned tsecs)
{
     ...
     while (tsecs > 0)
          {
          if ( (*siop->s_rstat)(siop) & siop->rcv_vflowstate) /* check UART for uint8 ready         */
               return  ((*siop->s_read)(siop));  /* return uint8 if ready     */
     ...
     return s_inchar(siop);                   /* timed out--no uint8 received */
}
...
#else
         { 0, 0, NIL, 0, 0, 0},   /* null if not SIO supported */
#endif
#ifdef COM2SIO
...
void __sysintron(SIO *siop, RANK portnum)
{
     ...
     __sys_ilist[portnum].oldimr =  imr = _a_inport ((uint8*)IMR);
     imr = (uint8) (imr &  __sys_ilist[portnum].imask);
     _a_outport((uint8*) IMR, imr);                  
...
{
     __sys_disable();
     _a_outport( (uint8*)IMR, __sys_ilist[portnum].oldimr); /* restore Int.Mask Register */
     __sys_enable();
     _dos_setvect(__sys_ilist[portnum].inum, __sys_ilist[portnum].oldivect);  /* restore vector */
