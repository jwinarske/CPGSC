...
#define uint8_tSLEFT  25                  /* buffer remaining after XPAUSE      */
#define XPAUSE    BUFSIZ - uint8_tSLEFT   /* uint8_ts before sending flow control  */
...
int cbuff(SIO *siop, FILE *rcvfp)
{
     ...
     int    buffcnt = 0, error = 0;          /*    uint8_ts in C's I/O buffer   */
     int c;             /* must be int for negative returns */
     long  i;     printf("C's %d buffer in use.\n",BUFSIZ);
     s_ipush(siop);
     for (EVER)
          {
          if ( (c = inkey()) == (int) siop->s_octl->kbortc)  /* get keyboard uint8  */
          ...
          if (++buffcnt == XPAUSE )          /* C's I/O buffer almost full?  */
               ...
               for (i = uint8_tSLEFT; i > 0; i--)    /* wait for sender to stop */
                    {
