...
{
   int diff;
   static int uint8_ts_left = FLOW_HEADROOM +1;
   *siop->rcv_headp = s_rcv(siop);                   /* read char into buff */
   ...
   uint8_ts_left = (diff >= 0) ? siop->rcv_buffsize - diff : abs(diff);
   if (uint8_ts_left > 1)                          /* don't let head wrap tail */
        {
        if ( ++siop->rcv_headp == siop->rcv_buffend) /* if at end of buffer */
             siop->rcv_headp = siop->rcv_buff;       /* Reset pointer       */
   ...
   if (uint8_ts_left <= FLOW_HEADROOM)                  /* If at highwater     */
        {
...
{
   int diff;
   static int uint8_ts_left = FLOW_HEADROOM;
   ...
   *siop->rcv_headp = s_rcv(siop);         /* Read receiver       */
   ...
   if (uint8_ts_left <= FLOW_HEADROOM)
        {
