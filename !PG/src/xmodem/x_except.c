...
uint16_t x_except(SIO *siop, uint16_t exceptnum, uint16_t *ex_cntp, uint16_t maxexcept)
{
     ...
     extern int   paksize;              /* number of uint8_ts to xmit or rcv   */
     extern uint16_t (*r_errckp)();       /* pointer to the error check funct */
     extern char  *ckvname;             /* string indicating CRC or cksum   */
     extern uint8  csync;                /* "NAK" for checksum, 'C' for crc  */
     extern char *x_msg[];
     extern struct rcvpacket *rcvbuffp; /* pointer to receive packet buffer */
     int errval = CONTINUE;
     switch (exceptnum)
...
/* errors in reception */
          case E_NOSOH:                 /* timeout waiting for SOH      */
               ...
               if (rcvbuffp->pnum1 == VIRGIN) /* negotiate startup mode */
                    ...
                    csync    = (uint8) ((r_errckp == x_rcvcrc) ? 'C'        : NAK);
