...
uint16_t makepacket(struct sndpacket *pbp, uint16_t pakcnt, ULONG paknum, FILE  *fp)
{
     ...
     while (fread(pbp->data, sizeof(uint8), sizeof(pbp->data), fp) != 0)
          {
          pbp->soh   = SOH;                  /* install SOH                  */
          pbp->pnum1 = (uint8)(paknum++ & 0x00FF); /* install packet number and ...*/
          pbp->pnum2 = (uint8)(~(pbp->pnum1));     /* its one's complement         */
          if (++paksread == pakcnt)
               break;
