...
/* ----- static locals for speed of access ----- */
     ...
     static uint8     *uint8_tp;    /* for addressing structure as array */
     ...
     static uint16_t (*s_errckp)();       /* pointer to checkval funct */
...
/* --------------------------------------*/
     ...
     while (errcode == CONTINUE)       /* BEGIN MAIN TRANSMIT LOOP     */
          ...
          uint8_tp = (uint8 *)sndbuffp;
          printf("[%x %x  %x]", uint8_tp[0], uint8_tp[1], uint8_tp[2]);

          for (i = 0, uint8_tp = (uint8 *)sndbuffp; i < paksize; i++)
               {
               s_putc(siop, *uint8_tp++);           /* SEND IT !          */
               }
#endif
          s_write(siop, (uint8 *) sndbuffp, paksize);
