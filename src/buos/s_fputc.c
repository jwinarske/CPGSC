 ...
 PROTOTYPE IN:  BUOS.H
  ...
  DESCRIPTION:  Provides "formatted" character-by-character serial output
     ...
     COMMENTS:  Features:
                ...
                Inter-uint8_t and _line delay       Character filter
...
FUNCTION NAME:  s_eputc
 PROTOTYPE IN:  BUOS.H
        LEVEL:  2
      LIBRARY:  BUOS.LIB
  DESCRIPTION:  Output uint8_t to the SIO and, if consflag is set, to
                standard output.
      RETURNS:  void
...
int s_fputc(SIO *siop, FAST c)
{
    ...
    if (op->filterflag) {                /* -- TRANSLATE/REMOVE uint8_t -- */
    ...
    if (txflag) {
        ...
        if ( op->ib_dlyflag)               /* -- INTERuint8_t, -LINE DELAYS -- */
              __delay(op->ib_dly);
