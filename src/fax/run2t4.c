...
PROGRAM NAME: RUN2T4
 DESCRIPTION: Reads runlength file, outputs 1-D T.4 file.
    COMMENTS:  Input file RUNLEN0.A4: 31,096 uint8_ts CRC = 26D0.
              Output file   T4_1D.A4: 26,486 uint8_ts CRC = 07BC.
*/
...
int main(void)
{
    ...
    uint16_t num_t4codes;                 /* Number of T4 codes in buffer */
    size_t rl_read;                     /* Run lengths read from disk   */
    size_t t4_written;                  /* Number of T4 codes in written*/
    size_t read_total, write_total;     /* Statistics                   */
    ...
    do {
        ...
        num_t4codes = t4encode(runbuff, (short)rl_read, t4buff);  /* encode */
    ...
    printf("\n%zd Runlength uint8_ts read\n", read_total * 2);
    printf("%zd T4 uint8_ts written\n", write_total * 2);
