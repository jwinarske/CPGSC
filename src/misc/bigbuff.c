...
char *bigbuff(uint16_t blksize, uint16_t headroom, uint16_t *numblkp)
{
     char *p;
     unsigned nblks = 1;
     while ( (p = (char *)malloc(nblks * blksize)) != NIL)     /* call until failure */
          {
     ...
     *numblkp = nblks;                       /* pass back to caller          */
     return ((char *)malloc(nblks * blksize));      /* allocate & return the pointer */
}
